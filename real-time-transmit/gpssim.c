             ...
             unsigned long sbf[5][N_DWRD_SBF])
{
    ...
    // FIXED: This has to be the "transmission" week number, not for the ephemeris
    // reference time
    ...
    sbf[0][1] = 0x1UL << 8;
    sbf[0][2] =
        ((wn & 0x3FFUL) << 20) | (ura << 14) | (((iodc >> 8) & 0x3UL) << 6);
    sbf[0][3] = 0UL;
    sbf[0][4] = 0UL;
    sbf[0][5] = 0UL;
                  ...
                  double xyz[])
{
    ...
    // manipulate range here
    double correction;

    //memcpy(&correction, &corrections[channel_id], sizeoff(double));
 ...
 */
void computeCodePhase(channel_t *chan, range_t rho1, double dt, int i)
{
    double ms;
    int ims;
    double rhorate;
    ...
    //memcpy(&chan->dataBit, &raw_bits[i], sizeof(int));
    
    chan->dataBit = i; //(int)((chan->dwrd[chan->iword] >> (29 - chan->ibit)) & 0x1UL) * 2 - 1;
...
int generateNavMsg(gpstime_t g, channel_t *chan, int init)
{
    ...
    if (init == 1) // Initialize subframe 5
    {
        ...
        for (iwrd = 0; iwrd < N_DWRD_SBF; iwrd++)
        {
            ...
            // Compute checksum
            sbfwrd |= (prevwrd << 30) &
                      0xC0000000UL; // 2 LSBs of the previous transmitted word
            nib = ((iwrd == 1) || (iwrd == 9))
                      ? 1
                      : 0; // Non-information bearing bits for word 2 and 10
...
int allocateChannel(channel_t *chan, ephem_t *eph, ionoutc_t ionoutc,
                    gpstime_t grx, double *xyz, double elvMask, struct hash_queue* hq)
{
            ...
            if (allocatedSat[sv] == -1) // Visible but not allocated
            {
                ...
                for (i = 0; i < MAX_CHAN; i++)
                {
                    if (chan[i].prn == 0)
                    {
                        ...
                        sprintf(sat_str,"%d", (sv+1));

                        if(!g_hash_table_contains(sm, g_strdup(sat_str)))     //// inserting to the hash table
                        {
                            printf("inserted sat %s : chan %s \n",(char *)sat_str, (char *)chan_str);
                            g_hash_table_insert(sm, g_strdup(sat_str), g_strdup(chan_str));
                        }
                        if (g_hash_table_contains(sm,g_strdup(sat_str)) && i != atoi(g_hash_table_lookup(sm,g_strdup(sat_str))))       //// when the channel for the satellite changes
                        { 
                            g_hash_table_replace(sm, g_strdup(sat_str), g_strdup(chan_str));
                            printf("replace sat %s : chan %s \n",(char *)sat_str, (char *)chan_str);
                        }
    ...
    {   
        sprintf(sat_prn_str, "%d", i+1);

        if (g_hash_table_contains(sm, sat_prn_str))
            {
                int assigned_chan = atoi(g_hash_table_lookup(sm,sat_prn_str));
            }        
    }
    return (nsat);
}
...
//     printf("%s, ", value);
// }
// static void print_hash(gpointer key, gpointer value, gpointer userdata)
// {
//     printf("sats are %s \n", key);
...
// g_hash_table_foreach(sm, print_hash, &total);
////////////////////////////////////////////////////////////////////////////////////////////////
...
{   
    ...
    pthread_t th_bits;
    pthread_create(&th_bits, NULL, (void *)bitstreamer_thread, (void *)&hq);

    pthread_t th_range;
    pthread_create(&th_range, NULL, (void *)dt_thread, NULL);

    pthread_t th_loc;
    pthread_create(&th_loc, NULL, (void *)locations_thread, NULL);
    
    //sleep(30);

    /// DEBUG FILE OUTPUT - RX Bit
    FILE *myfp2;
    myfp2 = fopen("debug_rx_bit_from_stream_ch0", "a+");
    ...
    else
    {
        ...
        printf("Using assigned_chan location mode.\n");
        llh2xyz(llh, xyz[0]); // Convert llh to xyz

        //memcpy(llhr, llh, 3 * sizeof(double)); // Assign static llh to udp received coordinates (these coordinates will be used until UDP socket receives new coordinates)
    ...
    if (ionoutc.vflg == TRUE)
    {
        ...
        printf("   %19.11e %19.11e  %9d %9d\n", ionoutc.A0, ionoutc.A1, ionoutc.tot,
               ionoutc.wnt);
        printf("%6d\n", ionoutc.dtls);
    }
    ...
    printf("tmin = %4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)\n", tmin.y, tmin.m,
           tmin.d, tmin.hh, tmin.mm, tmin.sec, gmin.week, gmin.sec);
    printf("tmax = %4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)\n", tmax.y, tmax.m,
           tmax.d, tmax.hh, tmax.mm, tmax.sec, gmax.week, gmax.sec);

    printf("Start time = %4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)\n", t0.y, t0.m,
           t0.d, t0.hh, t0.mm, t0.sec, g0.week, g0.sec);
    ...
    allocateChannel(chan, eph[ieph], ionoutc, grx, xyz[0], elvmask, &hq);
    ...
    for (iumd = 1; iumd < numd; iumd++)
    {
        ...
        for (i = 0; i < MAX_CHAN; i++)
        {
            if (chan[i].prn > 0)
            {
                ...
                computeCodePhase(&chan[i], rho, 0.1, dataBit[i]);
                ...
                if (chan[i].prn > 0)
                {
                    ...
                    if (chan[i].code_phase >= CA_SEQ_LEN)
                    {
                        ...
                        if (chan[i].icode >= 20) // 20 C/A codes = 1 navigation data bit
                        {
                            ...
                            // Set new navigation data bit from stream
                            int dbit = 0;

                            if (!g_queue_is_empty(queues[i]))
                            {
                                dbit = atoi(g_queue_pop_head(queues[i]));
                                chan[i].dataBit = dbit;
                                //printf("Rex Chn %d : %d : %d\n", i, chan[i].prn, dbit);
                            }
                            else
                            {
                                dbit =0;
                            ...
                            dataBit[i] = dbit;
                            if (tow_fix && local_fix) //<---- FIX THIS
                            {
                                double dt1, correction;

                                memcpy(&correction, &tow_correction, sizeof(double));

                                dt1 = correction - grx.sec;
                                printf("\nTOW Fix: %f\n", dt1);

                                grx = incGpsTime(grx, dt1);
                                local_fix = false;
                                tow_fixed = true;
                            }
        ...
        memcpy(&(s->fifo[s->head * 2]), iq_buff,
               NUM_IQ_SAMPLES * 2 * sizeof(int16_t));
        ...
        if (igrx % 300 == 0) // Every 30 seconds
        {
            ...
            allocateChannel(chan, eph[ieph], ionoutc, grx, xyz[iumd], elvmask, &hq);
            ...
            if (verb)
            {
                ...
                printf("%4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)\n", t0.y, t0.m, t0.d,
                       t0.hh, t0.mm, t0.sec, grx.week, grx.sec);
                printf("xyz = %11.1f, %11.1f, %11.1f\n", xyz[iumd][0], xyz[iumd][1],
                       xyz[iumd][2]);
        ...
        memcpy(&dt1, &dynamic_dt, sizeof(double));
